...
m_ihm.ui.cB_AX_type_cde->addItem("Vitesse",QVariant(cSERVO_AX_VITESSE));
 ...
 m_ihm.ui.cB_Asser->addItem("DistAng",QVariant(2));
  m_ihm.ui.cB_Asser->addItem("Manuel",QVariant(3));
 m_ihm.ui.cB_Asser->addItem("Init_asser",QVariant(4));
 m_ihm.ui.cB_Asser->addItem("Init_rack",QVariant(5));
 m_ihm.ui.cB_Asser->addItem("Rack",QVariant(6));
...
void CActuatorSequencer::updateTooltip(void)
{
 // Gestion des alias sur les noms et remplissage des combobox
    ...
    if(m_ihm.ui.cB_Asser==wdgt)
    {
        ...
        if(currentAsser=="Manuel")
        {
             //qDebug() << "asser text is" << currentAsser;
            m_ihm.ui.label_asserValue_1->setText("Droite");
            m_ihm.ui.label_asserValue_2->setText("Gauche");
            m_ihm.ui.label_asserValue_3->setText("Not Used");
            m_ihm.ui.lE_Asser_1->setValue(0);
            m_ihm.ui.lE_Asser_1->setMaximum(100);
            m_ihm.ui.lE_Asser_1->setMinimum(-100);
            m_ihm.ui.lE_Asser_2->setValue(0);
            m_ihm.ui.lE_Asser_2->setMaximum(100);
            m_ihm.ui.lE_Asser_2->setMinimum(-100);
            m_ihm.ui.lE_Asser_3->setValue(0.0);
            m_ihm.ui.lE_Asser_1->setEnabled(true);
            m_ihm.ui.lE_Asser_2->setEnabled(true);
            m_ihm.ui.lE_Asser_3->setEnabled(false);
        ...
        if(table_sequence->rowCount()>0)
        {
            ...
            //for (indexItem=idxMin;indexItem<idxMax+1;indexItem++)
            {
                ...
                if(sActuator.compare("Asser")==0) //ASSERVISSEMENT
                {
                    ...
                    if(id.compare("Manuel")==0)
                    {
                        if(nb_args==2)
                        {
                            msg="Commande Manuelle (D,G) = ("+value+")";
                            setPlayMessage(chrono.elapsed(),"ACTION",msg);
                            m_application->m_data_center->write("COMMANDE_MVT_MANUEL_TxSync", 1);
                            m_application->m_data_center->write("PuissanceMotD", args.at(0).toFloat());
                            m_application->m_data_center->write("PuissanceMotG", args.at(1).toFloat());
...
void CActuatorSequencer::Slot_Play_only_asser()
{
    ...
    if(id=="Manuel")
        if(nb_args==2)
        {
            qDebug() << "MVT Manuel" << float_values;
            m_application->m_data_center->write("COMMANDE_MVT_MANUEL_TxSync", 1);
            m_application->m_data_center->write("PuissanceMotD",float_values.at(0));
            m_application->m_data_center->write("PuissanceMotG", float_values.at(1));
            m_application->m_data_center->write("COMMANDE_MVT_MANUEL_TxSync", 0);
        }
            ...
            case ASSER:
                ...
                if(isSym)
                {
                    ...
                    if((sId.compare("Manuel")==0)&&(nb_args>=2))
                        sConverted=sConverted+(isProto?strProto:"")+QString("Application.m_asservissement.CommandeManuelle(%1,%2);/*%3*/").arg(args.at(0)).arg(args.at(1)).arg(sComments);
